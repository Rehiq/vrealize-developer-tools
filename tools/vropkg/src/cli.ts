...
async function run() {
	...
	switch (t.ProjectType[input.in]) {
		case t.ProjectType.tree:               pkgPromise = parseTree(input.srcPath);                   break;
		case t.ProjectType.flat:               pkgPromise = parseFlat(input.srcPath, input.destPath);   break;
		case t.ProjectType.js:                 pkgPromise = new VroJsProjParser().parse(input.srcPath); break;
	...
	switch (t.ProjectType[input.out]) {
		case t.ProjectType.tree:                serializeTree(pkg, input.destPath);                         break;
		case t.ProjectType.flat:                serializeFlat(pkg, input.destPath);                         break;
		case t.ProjectType.js:                  await new VroJsProjRealizer().realize(pkg, input.destPath); break;
